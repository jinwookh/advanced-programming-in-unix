...
static off_t _db_readptr(DB *db, off_t offset) {
	...
	if (lseek(db->idxfd, offset, SEEK_SET) == -1)
		printf("_db_readptr: lseek error");
	if (read(db->idxfd, asciiptr, PTR_SZ) != PTR_SZ)
		printf("_db_readptr: read error");
	asciiptr[PTR_SZ] = 0;
...
static off_t _db_readidx(DB * db, off_t offset) {
	...
	*ptr2++ = 0;
	...
	if (strchr(ptr2, SEP) != NULL)
		printf("_db_readidx: too many separator");
...
static void _db_writeidx(DB *db, const char * key, off_t offset, int whence, off_t ptrval) {
	...
	if (writev(db->idxfd, &iov[0], 2) != PTR_SZ + IDXLEN_SZ + len)
		printf("_db_writeidx: writev error");
...
static void _db_writeptr(DB *db, off_t offset, off_t ptrval) {
	...
	if (ptrval < 0 || ptrval > PTR_MAX)
		printf("_db_writeptr: invalid ptr: %lld", ptrval);
